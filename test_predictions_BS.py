...
## General Definitions
test_dir = '/Users/amograo/Desktop/Bone_Suppressed_Test'
dest_dir = '/Users/amograo/Desktop/Pn_Research/se_resnext50_32x4d_BS/test_files'
model_path = '/Users/amograo/Desktop/Pn_Research/se_resnext50_32x4d_BS/models'
...
def pred_to_csv(test_dir,dest_dir,model_path,model_number,ENCODER=ENCODER):
    ...
    for image in tqdm(images):
        ...
        image_array=cv2.imread(os.path.join(test_dir,image))
        image_array_512=cv2.cvtColor(image_array,cv2.COLOR_BGR2RGB)
        ...
        # predicted_mask = (torch.sigmoid(predicted_mask) >= 0.4).int()
        pr_mask = (predicted_mask.squeeze().cpu().numpy().round())

        # mega_list.extend(list(set(pr_mask.flatten())))
    ...
    meta_df.to_csv(f'{dest_dir}/U-{ENCODER}_{model_number}_BS_0.4.csv',index=False)
